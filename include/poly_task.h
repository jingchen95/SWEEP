...
// #endif
  ...
  virtual void set_lp_task_distri_state(bool new_state) = 0;
  virtual bool get_lp_task_distri_state() = 0;
  virtual void set_hp_task_distri_state(bool new_state) = 0;
  virtual bool get_hp_task_distri_state() = 0;
  virtual void set_clus0_cpu_freq(int freq_index) = 0;
  virtual void set_clus1_cpu_freq(int freq_index) = 0;
  virtual int get_clus0_cpu_freq() = 0;
  virtual int get_clus1_cpu_freq() = 0;
#ifdef ALLOWSTEALING
...
// #endif
  virtual int JOSS_for_Energy(int nthread, PolyTask * it);
  virtual int EPTO_for_EDP(int nthread, PolyTask * it);
  ...
  virtual int AAWS_HP(int nthread, PolyTask * it, int DOP);
  virtual int AAWS_LP(int nthread, PolyTask * it, int DOP);
  virtual int task_distri_HP(int nthread, PolyTask * it, int DOP);     // Paper 4: decide the resource width and distribute DOP task among clusters (high parallelism)
  virtual int task_distri_LP(int nthread, PolyTask * it, int DOP);     // Paper 4: decide the resource width and distribute DOP task among clusters (low parallelism)
  virtual int assign_config(int nthread, PolyTask * it);      // Paper 4: assign the best config to the current ready task but not scheduled yet
