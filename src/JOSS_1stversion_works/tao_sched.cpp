...
int gotao_init_hw( int nthr, int thrb, int nhwc){
  ...
  // }
  SupportedCPUFreq.open("HW_Supported_CPU_Freq");
  SupportedDDRFreq.open("HW_Supported_DDR_Freq");
...
*/  
  ...
  infile1.open("HW_idle_CPUPower"); /* Step 1: Read idle CPU power */
  if(infile1.fail()){
  ...
  infile1.open("HW_idle_DDRPower"); /* Step 2: Read idle DDR power */
  if(infile1.fail()){
    std::cout << "Failed to open power profile file!" << std::endl;
    std::cin.get();
    return 0;
              ...
              for(auto&& width : ptt_layout[START_CLUSTER_B]) { 
                ...
                if(check_ticks > 0.0f && assembly->get_PTT_UpdateFinish(1, 1, width-1) >=NUM_TRAIN_TASKS){
                  ...
                  if(assembly->get_mbtable(1, width-1) == 0.0f){
                    ...
                    if(memory_boundness > 1){
                      LOCK_ACQUIRE(output_lck);
                      std::cout << "[Warning] Memory-boundness Calculation (ClusterB) is greater than 1!" << std::endl;
                      LOCK_RELEASE(output_lck);
                      memory_boundness = 1;
                    }else{
                      if(memory_boundness <= 0){ /* Execution time and power prediction according to the computed memory-boundness level */
                        LOCK_ACQUIRE(output_lck);
                        std::cout << "[Warning] Memory-boundness Calculation (ClusterB) is smaller than 0!" << std::endl;
                        LOCK_RELEASE(output_lck);
                        memory_boundness = 0.00001;
                      }
                    }
...
#ifdef DEBUG
              LOCK_ACQUIRE(output_lck);
              std::cout << "[DEBUG] " << assembly->kernel_name << ": ClusterB cluster completed PTT training at " << TRAIN_MAX_B_FREQ << " and " << TRAIN_MED_B_FREQ << "\n";
              LOCK_RELEASE(output_lck);
#endif
            }
          }
